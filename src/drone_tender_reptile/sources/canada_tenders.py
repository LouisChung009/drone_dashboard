"""CanadaBuys tender notice open-data fetcher (CSV downloads).

Data Source:
- Canada: CanadaBuys tender notice CSV exports
"""
...
class CanadaTenderCsvFetcher(BaseFetcher):
    source_name = "canada-tenders"

    def fetch(self) -> Iterable[TenderRecord]:
        urls: List[str] = self.config.get("download_urls", [])
        if not urls:
            raise RuntimeError("Canada tender configuration requires download_urls")
    ...
    def _iter_csv(self, url: str):
        logger.info("Downloading CanadaBuys tender CSV %s", url)
        with requests.get(
            url,
    ...
    def _normalize(self, row: dict) -> Optional[TenderRecord]:
        ...
        return TenderRecord(
            ...
            title=title,
            description=row.get("tenderDescription-descriptionAppelOffres-eng")
            ...
            award_date=None,
            supplier_name=None,
            data_source_url=row.get("noticeURL-URLavis-eng")
